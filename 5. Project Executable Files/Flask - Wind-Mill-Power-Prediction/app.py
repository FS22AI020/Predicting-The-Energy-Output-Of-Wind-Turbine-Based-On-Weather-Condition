...
loaded_model = joblib.load('Project\power_prediction.sav')
scale_model = joblib.load('Project\scaler.pkl')

def format_to_two_decimals(value):
    return f"{value:.2f}"

if not os.path.isfile('login.csv'):
    with open(r'Project\Flask - Wind-Mill-Power-Prediction\login.csv', newline='') as file:
...
app.config['MAIL_USERNAME'] = 'fs22ai020ayush@gmail.com'  # replace with your email
app.config['MAIL_PASSWORD'] = 'mejd jzpr zohg nurf' 
...
def register():
    ...
    try:
        with open(r'Project\Flask - Wind-Mill-Power-Prediction\login.csv') as file:
